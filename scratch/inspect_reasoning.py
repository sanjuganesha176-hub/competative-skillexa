# scratch/inspect_reasoning.py
import json

data = json.load(open('server/data/skillexa.json', encoding='utf-8'))
for c in data.get('courses', []):
    if 'reason' in c.get('title', '').lower() or 'reason' in c.get('slug', '').lower():
        print('Course:', c)
        ...
        for m in modules:
            ...
            for t in sorted(topics, key=lambda x: x.get('order_index', 0)):
                ...
                print(f"  Topic {tid:02d} (Order {t.get('order_index')}): {t.get('title'):<26} | Lessons: {len(lessons)} | PYQs: {len(pyqs)} | Practice: {len(practice)} | QuizQs: {len(quiz)}")
