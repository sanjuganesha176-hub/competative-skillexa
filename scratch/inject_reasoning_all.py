# scratch/inject_reasoning_all.py
import json
import os
import sys
...
non_reasoning_lessons = [l for l in db.get('lessons', []) if not (38 <= l.get('topic_id', 0) <= 51)]
new_reasoning_lessons = []
for tid in sorted([int(k) for k in all_reasoning.keys()]):
    m = all_reasoning[str(tid)]
    new_reasoning_lessons.append({
        "id": 100 + tid,
        "topic_id": tid,
        "title": m["title"],
...
db['lessons'] = non_reasoning_lessons + new_reasoning_lessons
print(f"Lessons updated: {len(non_reasoning_lessons)} non-reasoning + {len(new_reasoning_lessons)} reasoning = {len(db['lessons'])} total.")

# 2. Update/Add Previous Year Questions
non_reasoning_pyqs = [p for p in db.get('previous_year_questions', []) if not (38 <= p.get('topic_id', 0) <= 51)]
new_reasoning_pyqs = []
for tid in sorted([int(k) for k in all_reasoning.keys()]):
    m = all_reasoning[str(tid)]
...
db['previous_year_questions'] = non_reasoning_pyqs + new_reasoning_pyqs
print(f"PYQs updated: {len(non_reasoning_pyqs)} non-reasoning + {len(new_reasoning_pyqs)} reasoning = {len(db['previous_year_questions'])} total.")

# 3. Update/Add Practice Questions
non_reasoning_pqs = [p for p in db.get('practice_questions', []) if not (38 <= p.get('topic_id', 0) <= 51)]
new_reasoning_pqs = []
for tid in sorted([int(k) for k in all_reasoning.keys()]):
    m = all_reasoning[str(tid)]
...
db['practice_questions'] = non_reasoning_pqs + new_reasoning_pqs
print(f"Practice Questions updated: {len(non_reasoning_pqs)} non-reasoning + {len(new_reasoning_pqs)} reasoning = {len(db['practice_questions'])} total.")

# 4. Update/Add Quiz Questions
non_reasoning_qqs = [q for q in db.get('quiz_questions', []) if not (38 <= q.get('topic_id', 0) <= 51)]
new_reasoning_qqs = []
for tid in sorted([int(k) for k in all_reasoning.keys()]):
    m = all_reasoning[str(tid)]
...
db['quiz_questions'] = non_reasoning_qqs + new_reasoning_qqs
print(f"Quiz Questions updated: {len(non_reasoning_qqs)} non-reasoning + {len(new_reasoning_qqs)} reasoning = {len(db['quiz_questions'])} total.")
